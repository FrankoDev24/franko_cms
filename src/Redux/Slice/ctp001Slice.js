...
// These map to whatever operation flag your API expects.
// Adjust the values ("merge", "update", "unmerge") to match your backend contract.
...
export const getCTP001Products = createAsyncThunk(
  ...
  async ({ pageNumber = 1, recordPerPage = 2000 } = {}, { rejectWithValue }) => {
    try {
      ...
      const explicitTotal =
        rawData?.total ?? rawData?.totalCount ?? rawData?.count ?? rawData?.totalRecords;
...
// ── Find Similar CTP002 Products (client-side) ──
export const findSimilarCTP002Products = createAsyncThunk(
  ...
  async (ctp001Product, { rejectWithValue, getState }) => {
    try {
      ...
      if (!ctp002Products.length) return { candidates: [], reason: "no_ctp002_loaded" };

      const ctp001Name = getProductName(ctp001Product);
      if (!ctp001Name) return { candidates: [], reason: "no_name" };
      ...
      const candidates = ctp002Products
        .map((ctp2) => {
          ...
          return {
            ...
            similarity,
            similar: areNamesSimilar(ctp001Name, ctp2Name),
          };
        })
        .filter((c) => c.similar)
...
/* ══════════════════════════════════════════════════════════════════
   CORE: MergeNUpdateCTO001_N_CTP002Products
   Handles: merge (new link), update (change existing link), unmerge
══════════════════════════════════════════════════════════════════ */

/**
 * Shared internal caller for the /MergeNUpdateCTO001_N_CTP002Products endpoint.
 * `operation` is one of MERGE_OPERATION.MERGE | .UPDATE | .UNMERGE
 *
 ...
 *     ctP001ProductId,
 *     ctP001ProductName,
 *     ctP002ProductId,      // empty string when unmerging (if API requires it)
 *     ctP002ProductName,    // empty string when unmerging
 *     operation,            // "merge" | "update" | "unmerge"
 *   }
 * ]
 *
 * Adjust field names / shape to match your actual backend contract.
 */
const callMergeEndpoint = async (payload) => {
  const res = await axiosInstance.post("/", payload, {
    params: { endpoint: "/MergeNUpdateCTO001_N_CTP002Products" },
    ...
    try {
      ...
      if (!ctp1Id)
        return rejectWithValue(
          `Missing Sales Mate Product ID. Fields: ${Object.keys(ctp001Product || {}).join(", ")}`
        );
      if (!ctp2Id)
        return rejectWithValue(
          `Missing Website Product ID. Fields: ${Object.keys(ctp002Product || {}).join(", ")}`
        );
      ...
      const rawData = await callMergeEndpoint(payload);
      const msg = getResponseMessage(rawData) || "Products linked successfully";
      ...
      return rejectWithValue(toErrorPayload(error, "Failed to link products"));
    }
  }
);

// ── Update (re-link to a different website product) ──
export const updateMergedProduct = createAsyncThunk(
  "ctp001/updateMergedProduct",
  async ({ ctp001Product, newCtp002Product }, { rejectWithValue }) => {
    try {
      const ctp1Id = String(getProductId(ctp001Product) || "").trim();
      const ctp2Id = String(getProductId(newCtp002Product) || "").trim();
      const ctp1Name = getProductName(ctp001Product);
      const ctp2Name = getProductName(newCtp002Product);

      if (!ctp1Id) return rejectWithValue("Missing Sales Mate Product ID.");
      if (!ctp2Id) return rejectWithValue("Missing new Website Product ID.");
      ...
      const payload = [
        {
          ...
          ctP002ProductId: ctp2Id,
          ctP002ProductName: ctp2Name,
          operation: MERGE_OPERATION.UPDATE,
        },
      ];

      const rawData = await callMergeEndpoint(payload);
      const msg = getResponseMessage(rawData) || "Link updated successfully";
      ...
      if (!error.response)
        return rejectWithValue("Network error: Could not reach the server.");
      return rejectWithValue(toErrorPayload(error, "Failed to update link"));
    }
  }
);

// ── Unmerge (remove link) ──
export const unmergeCTP001Product = createAsyncThunk(
  "ctp001/unmergeCTP001Product",
  async ({ ctp001Product, ctp002Product }, { rejectWithValue }) => {
    try {
      const ctp1Id = String(getProductId(ctp001Product) || "").trim();
      const ctp2Id = String(getProductId(ctp002Product) || "").trim();
      ...
      // Some APIs still want both IDs even for unmerge; adjust as needed.
      const payload = [
        {
          ctP001ProductId: ctp1Id,
          ctP001ProductName: ctp1Name,
          ctP002ProductId: ctp2Id || "",
          ctP002ProductName: ctp2Name || "",
          operation: MERGE_OPERATION.UNMERGE,
        },
      ];

      const rawData = await callMergeEndpoint(payload);
      const msg = getResponseMessage(rawData) || "Link removed successfully";
      ...
      if (!error.response)
        return rejectWithValue("Network error: Could not reach the server.");
      return rejectWithValue(toErrorPayload(error, "Failed to remove link"));
    }
  }
);

// ── Bulk Auto-Merge (exact name match) ──
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      ctp001Products.forEach((ctp1) => {
        ...
        if (!ctp1Name) return;
        const matchedCtp2 = ctp002Products.find(
          (ctp2) =>
            areNamesSimilar(ctp1Name, getProductName(ctp2)) &&
            getSimilarity(ctp1Name, getProductName(ctp2)) === 1
        );
        if (matchedCtp2) {
          mergePairs.push({
      ...
      const rawData = await callMergeEndpoint(mergePairs);

      return {
        success: true,
        message: getResponseMessage(rawData) || "Bulk merge completed",
        products: toArray(rawData),
        nothingToMerge: toArray(rawData).length === 0,
        mergePairs,
      };
    } catch (error) {
      return rejectWithValue(toErrorPayload(error, "Failed to bulk merge products"));
    }
  }
);
...
const initialState = {
  ...
  mergedProductMap: {},   // { ctp001Id -> ctp002Id }
  manualMerges: [],
  similarCandidates: [],
  mergeResult: null,
  orders: [],
...
/* ════════════════════════════════════════════
   SHARED REDUCER: handle fulfilled merge/update
════════════════════════════════════════════ */

// Reused for both merge and update fulfilled cases
const applyMergeOrUpdate = (state, action) => {
  const { ctp001ProductId, ctp002ProductId, mergedPair, message: msg, operation } =
    action.payload;

  // Upsert the map entry
  state.mergedProductMap[ctp001ProductId] = ctp002ProductId;

  // Update manualMerges list (remove old entry first so no duplicates)
  state.manualMerges = state.manualMerges.filter(
    (m) => m.ctp001ProductId !== ctp001ProductId
  );
  ...
  // Also keep mergedProducts list in sync
  const existingIdx = state.mergedProducts.findIndex(
    (m) =>
      (m.ctP001ProductId || m.ctp001ProductId) === ctp001ProductId
  );
  const newEntry = {
    ctP001ProductId: ctp001ProductId,
    ctP001ProductName: mergedPair.ctP001ProductName,
    ctP002ProductId: ctp002ProductId,
    ctP002ProductName: mergedPair.ctP002ProductName,
  };
  if (existingIdx >= 0) {
    state.mergedProducts[existingIdx] = newEntry;
  } else {
    state.mergedProducts.push(newEntry);
  }

  state.mergeResult = {
    success: true,
    message: msg || (operation === MERGE_OPERATION.UPDATE ? "Link updated" : "Linked successfully"),
...
// Reused for unmerge fulfilled
const applyUnmerge = (state, action) => {
  const { ctp001ProductId, message: msg } = action.payload;

  delete state.mergedProductMap[ctp001ProductId];

  state.manualMerges = state.manualMerges.filter(
    (m) => m.ctp001ProductId !== ctp001ProductId
  );

  state.mergedProducts = state.mergedProducts.filter(
    (m) =>
      (m.ctP001ProductId || m.ctp001ProductId) !== ctp001ProductId
  );
  ...
  state.mergeResult = {
    success: true,
    message: msg || "Link removed successfully",
    nothingToMerge: false,
    count: 0,
    operation: MERGE_OPERATION.UNMERGE,
  };
...
const ctp001Slice = createSlice({
  ...
  reducers: {
    ...
    // Local-only unmerge (fallback / optimistic) — prefer using unmergeCTP001Product thunk
    removeManualMerge: (state, action) => {
      ...
      if (ctp001Id && state.mergedProductMap[ctp001Id]) {
        ...
        state.mergedProducts = state.mergedProducts.filter(
          (m) => (m.ctP001ProductId || m.ctp001ProductId) !== ctp001Id
        );
      }
    },
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      /* ── mergeSingleCTP001WithCTP002 (new link) ── */
      ...
      .addCase(mergeSingleCTP001WithCTP002.fulfilled, (state, action) => {
        ...
        applyMergeOrUpdate(state, action);
      })
      .addCase(mergeSingleCTP001WithCTP002.rejected, (state, action) => {
        state.loading.singleMerge = false;
        state.error.singleMerge = action.payload || "Failed to link products";
        state.mergeResult = {
          success: false,
          message: action.payload || "Failed to link products",
          nothingToMerge: false,
          count: 0,
          operation: MERGE_OPERATION.MERGE,
        };
      ...
      /* ── updateMergedProduct (re-link) ── */
      .addCase(updateMergedProduct.pending, (state) => {
        state.loading.updateMerge = true;
        state.error.updateMerge = null;
      })
      .addCase(updateMergedProduct.fulfilled, (state, action) => {
        state.loading.updateMerge = false;
        applyMergeOrUpdate(state, action);
      })
      .addCase(updateMergedProduct.rejected, (state, action) => {
        state.loading.updateMerge = false;
        state.error.updateMerge = action.payload || "Failed to update link";
        state.mergeResult = {
          success: false,
          message: action.payload || "Failed to update link",
          nothingToMerge: false,
          count: 0,
          operation: MERGE_OPERATION.UPDATE,
        };
      ...
      .addCase(unmergeCTP001Product.pending, (state) => {
        state.loading.unmergeMerge = true;
        state.error.unmergeMerge = null;
      })
      .addCase(unmergeCTP001Product.fulfilled, (state, action) => {
        state.loading.unmergeMerge = false;
        applyUnmerge(state, action);
      })
      .addCase(unmergeCTP001Product.rejected, (state, action) => {
        state.loading.unmergeMerge = false;
        state.error.unmergeMerge = action.payload || "Failed to remove link";
      })
      ...
      .addCase(mergeCTP001AndCTP002Products.fulfilled, (state, action) => {
        ...
        // Bulk-update the map from mergePairs
        (result.mergePairs || []).forEach((pair) => {
          if (pair.ctP001ProductId && pair.ctP002ProductId) {
            state.mergedProductMap[pair.ctP001ProductId] = pair.ctP002ProductId;
          }
        });
      ...
      .addCase(mergeCTP001AndCTP002Products.rejected, (state, action) => {
        state.loading.mergeAction = false;
        state.error.mergeAction = action.payload || "Failed to bulk merge products";
        state.mergeResult = {
          success: false,
          message: action.payload || "Failed to bulk merge products",
          nothingToMerge: false,
          count: 0,
          operation: MERGE_OPERATION.MERGE,
        };
      ...
      .addCase(getMergedProducts.fulfilled, (state, action) => {
        ...
        state.mergedProducts = fetched;

        // Rebuild map from server truth
        const newMap = {};
        fetched.forEach((m) => {
...
export const selectUpdateMergeLoading = (state) => state.ctp001.loading.updateMerge;
export const selectUnmergeLoading = (state) => state.ctp001.loading.unmergeMerge;
...
export const selectUpdateMergeError = (state) => state.ctp001.error.updateMerge;
export const selectUnmergeError = (state) => state.ctp001.error.unmergeMerge;
